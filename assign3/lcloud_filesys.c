...
typedef struct {
    ...
    int position;
    int length;
    int blocks[40][3];  //order of indices is sector, block, device array index
} File;
...
int lcread( LcFHandle fh, char *buf, size_t len ) {
    ...
    for (int read = 0; read < total_possible_reads; read++) {
        ...
        /*cache_buffer = lcloud_getcache(active_devices_array[device_index].id, temp_sector, temp_block);
        ...
        if (cache_buffer == '-1') {
            ...
            for (int index = 0; index < 256; index++) {
                ...
                buffer[index] = cache_buffer[index];
            }
        }*/
        get_block(buffer, active_devices_array[device_index].id, temp_sector, temp_block);
...
int lcwrite( LcFHandle fh, char *buf, size_t len ) {
    ...
    for (int write = 0; write < total_possible_writes; write++) {
        ...
        //lcloud_putcache(active_devices_array[device_index].id, sector, block, buffer);
...
int lcclose( LcFHandle fh ) {
    ...
    for (int i = 0; i < 40; i++) {
        if (open_files_array[location].blocks[i][0] != -1) {
            ...
            logMessage(LcDriverLLevel, "Deallocated block for data [%d/%d/%d]", open_files_array[location].blocks[i][2], open_files_array[location].blocks[i][0], open_files_array[location].blocks[i][1]);
        }
